...
func (tx *Tx) init() {
	tx.Common = NewCommonClient(tx.config)
	tx.WinBetslip = NewWinBetslipClient(tx.config)
	tx.WinBetslipsDetail = NewWinBetslipsDetailClient(tx.config)
	tx.WinCoinLog = NewWinCoinLogClient(tx.config)
	tx.WinConfig = NewWinConfigClient(tx.config)
	tx.WinDict = NewWinDictClient(tx.config)
	tx.WinDictItem = NewWinDictItemClient(tx.config)
	tx.WinGameList = NewWinGameListClient(tx.config)
	tx.WinGameSlot = NewWinGameSlotClient(tx.config)
	tx.WinPlatList = NewWinPlatListClient(tx.config)
	tx.WinUser = NewWinUserClient(tx.config)
	tx.WinUserWallet = NewWinUserWalletClient(tx.config)
}
...
// applies a query, for example: Common.QueryXXX(), the query will be executed
